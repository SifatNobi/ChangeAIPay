...
async function balance(req, res) {
  try {
    ...
    return res.status(502).json({ error: "Failed to fetch balance via Nano RPC", details: String(err?.message || err) });
  }
}
...
async function sendPayment(req, res) {
  try {
    ...
    if (!amountNano) return res.status(400).json({ error: "Amount is required" });

    const sender = await User.findById(req.user.id).select("+walletId");
    if (!sender) return res.status(404).json({ error: "User not found" });
    if (!sender.walletAddress || !sender.walletId) return res.status(400).json({ error: "Wallet not created" });
    ...
    if (!receiver.walletAddress) return res.status(400).json({ error: "Recipient has no wallet" });
    if (String(receiver._id) === String(sender._id)) return res.status(400).json({ error: "Cannot send to yourself" });
    ...
    return res.status(502).json({ error: "Failed to send via Nano RPC", details: String(err?.message || err) });
  }
}
